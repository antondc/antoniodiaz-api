...
export class LanguageUpdateOneUseCase implements ILanguageUpdateOneUseCase {
  ...
  public async execute(languageUpdateRequest: ILanguageUpdateOneRequest): Promise<ILanguageUpdateOneResponse> {
    ...
    const { richContentJson, richContentHtml } = richContent.generateHtml(whoContentJson);

    await this.languageRepo.languageGlossaryUpdateOne({ id: languageExists?.id, who, whoContentJson: richContentJson, whoContentHtml: richContentHtml });
