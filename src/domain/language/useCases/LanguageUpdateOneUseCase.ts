...
export class LanguageUpdateOneUseCase implements ILanguageUpdateOneUseCase {
  ...
  public async execute(languageUpdateRequest: ILanguageUpdateOneRequest): Promise<ILanguageUpdateOneResponse> {
    const { session, slug, glossary } = languageUpdateRequest;
    ...
    const languageToSave = new Language({ glossary });
    await this.languageRepo.languageGlossaryUpdateOne({ id: languageExists?.id, glossary: languageToSave.glossary });
