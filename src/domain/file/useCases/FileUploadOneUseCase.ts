...
export class FileUploadOneUseCase implements IFileUploadOneUseCase {
  ...
  public async execute(imageUploadOneRequest: IFileUploadOneRequest): Promise<IFileUploadOneResponse> {
    const { fileBlob } = imageUploadOneRequest;
    if (!allowedFileExtensions.includes(fileBlob.extension)) throw new RequestError('Wrong file extension', 400);

    const fileInstance = new File({ fileBlob, fileRepo: this.fileRepo });

    const mimeIncludesSomeMimeFileType = allowedFileExtensions.some((item) => fileBlob.type.toLowerCase().includes(item.toLowerCase()));
    if (!mimeIncludesSomeMimeFileType) throw new RequestError('Wrong file type', 400);

    const { path } = await fileInstance.fileSaveInTempFolder({ fileBlob });
